...
for((iB=0;iB<${#Bvals_fem[@]};iB++)); do
    ...
    for((lfem=${mmax};;lfem+=2)); do
		   ...
		   load=""
               ...
               atomic --Z=${atom} --method=HF --lmax=${lfem} --mmax=${mmax} --Rmax=100 --nelem=7 --M=$M --Bz=${Bvals_fem[iB]} --readocc=-1 --iguess=0 --save=fem_${lfem}_${iocc}.chk ${load} &> fem_${lfem}_${iocc}.log
           fi
           if(( lfem > mmax )); then
               let llast=lfem-2
