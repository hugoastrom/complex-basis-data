    ...
    for((iocc=0;;iocc++)); do
	   occfile="../../${atom}_${iocc}.occs"
           ...
           for basis in ${bases[@]}; do
               cat > real_${basis}_${iocc}.run <<EOF
System ../atoms.xyz
Basis ${basis}
Decontract *
Method HF
...
MaxInitIter 100
EOF
	       conv=$(grep "Converged to" real_${basis}_${iocc}.stdout)
               if [ -z "$conv" ]; then
		   echo ${atom} ${basis}_${iocc}
		   erkale_complex_orbs_omp real_${basis}_${iocc}.run &> real_${basis}_${iocc}.stdout
               fi
           done
       done
       done
